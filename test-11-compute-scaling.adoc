...
Selection (1-11)? 4
----
. If prompted, type yes. For example:
[source,console]
----
...
In most cases, you can leave these as defaults.
...
. Add/configure new network interfaces for your SVM on a per-node basis to add performance to the workload. Add at least one per node for NFSv3 and at least two per node if using NFSv4.x. (Up to 16)
+
[source,console]
----
