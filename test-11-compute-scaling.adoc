...
. Log into the service processor of the node(s) you wish to add to the cluster. From the prompt, type “system console” to access the node’s console. If the node is not at the LOADER prompt, you can use ctl+d to exit to the BMC/service processor prompt. Type “system reset”, answer y for dirty shutdown, then “system console” to watch the boot process. Hit ctl+c when you see the “interrupt Autoboot” message. Once at the LOADER prompt, run the following: set-defaults setenv bootarg.init.unjoined true saveenv Then, boot the node to the boot menu prompt.
+
[source,console]
----
