...
Navigate to Cluster -> Storage VMs +
 +
 +
Click on the desired SVM and look for the â€œProtocolsâ€ section. That shows if S3 is enabled. +
 +
 +
Click on the â€œSettingsâ€ tab and then the  on the S3 section. +
 +
 +
...
Click save. ONTAP will perform configurations behind the scenes, such as user creation, certificates, etc. +
 +
Once itâ€™s finished, a screen will pop up with information about the S3 server needed for authentication to the buckets. Download these or copy and save for future use. +
 +
 +
...
For root user, click the  and â€œRegenerate keyâ€ and then save the information. +
 +
 +
To add a user, click â€œAddâ€ and type the name and set the desired key validity. Remember, the user must map to a NAS user if you intend on leveraging NAS/Object protocol duality. +
 +
 +
...
Navigate to Storage -> Buckets and then click â€œAddâ€ and â€œMore optionsâ€ and add your name, SVM and map the bucket to a folder with the NAS data. +
  +
If only one Storage VM is present, you will not need to select the Storage VM. +
 +
Optionally, modify bucket permissions, users who have access, etc. Then click â€œSave.â€ +
  +
 +
