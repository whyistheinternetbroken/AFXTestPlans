...
This step may not be necessary if you’ve already peered the clusters for SnapMirror. +
...
Scroll down to Storage VM peers and click the  and select “Add a storage VM peer” +
 +
...
Click on Storage -> Volumes and click “Add” +
 +
...
Give the cache a name and select “Add as a cache for remote volume” below the name and select the cluster, SVM and volume to connect to as a cache. +
 +
Give the cache a size – it doesn’t have to be the same size as the origin volume – and configure the access permissions. +
...
In System Manager, on the FlexCache volume, click  and select “Prepopulate folders” +
 +
